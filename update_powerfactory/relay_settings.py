...
from update_powerfactory.logging_utils import get_logger, LogContext, log_performance
import external_variables as ev

logger = get_logger(__name__)


@log_performance(operation_name="Relay Settings")
def relay_settings(
        app,
        device_object: Any,
    ...
    with LogContext(device_name=device_name, operation="relay_settings"):
        ...
        if phase is not None:
            ...
            logger.debug(f"Set relay phase to {phase}", pf_output=False)
        ...
        if mapping_file:
            ...
            logger.warning(f"No mapping file found for pattern: {device_object.device}")
...
def update_relay_type(
    ...
    if not relay_type:
        logger.warning(f"Relay type not found: {mapping_type}", pf_output=False)
        return False

    pf_device.SetAttribute("e:typ_id", relay_type)
    pf_device.SlotUpdate()
    logger.debug(f"Updated relay type to: {mapping_type}", pf_output=False)
    return True
...
def apply_settings(
    ...
    for mapped_set in mapping_file:
        ...
        if not element:
            logger.error(f"Unable to find element for {mapped_set}", pf_output=True)
            continue
...
def update_logic_elements(
    ...
    for element in element_list:
        ...
        if not pf_element:
            logger.error(f"Logic element could not be found: {element}", pf_output=True)
            continue
        ...
        if len(existing_dip_set) != len(element_mapping):
            ...
            logger.warning(
                f"Dip switch count mismatch for {element}: "
                f"expected {len(element_mapping)}, got {len(existing_dip_set)}",
                pf_output=False
            )
            continue
