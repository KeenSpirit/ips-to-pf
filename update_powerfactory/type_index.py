...
class RelayTypeIndex:
    ...
    def build(cls, app) -> 'RelayTypeIndex':
        ...
        logger.info(f"Found {len(ergon_types or [])} relay types in ErgonLibrary")
        ...
        try:
            ...
            logger.info(f"Found {len(dig_types or [])} relay types in DIgSILENT library")
        except (IndexError, AttributeError) as e:
            logger.warning(f"Could not access DIgSILENT library: {e}")
...
class FuseTypeIndex:
    ...
    def build(cls, app) -> 'FuseTypeIndex':
        ...
        try:
            ...
            if not fuse_folder:
                logger.warning("Fuse folder not found in ErgonLibrary")
...
def get_relay_types(app) -> List[Any]:
    """
    Legacy function that returns relay types as a list.
    
    DEPRECATED: Use RelayTypeIndex.build(app) instead for O(1) lookups.
    ...
    """
    logger.warning(
        "get_relay_types() is deprecated. "
        "Use RelayTypeIndex.build(app) for O(1) lookups."
    )
    index = RelayTypeIndex.build(app)
    return index.get_all()


def get_fuse_types(app) -> List[Any]:
    """
    Legacy function that returns fuse types as a list.
    
    DEPRECATED: Use FuseTypeIndex.build(app) instead for O(1) lookups.
    This function is provided for backward compatibility only.
    ...
    """
    logger.warning(
        "get_fuse_types() is deprecated. "
        "Use FuseTypeIndex.build(app) for O(1) lookups."
    )
