...
def _create_info_dict(pf_device, result: str) -> Dict[str, str]:
    """
    Legacy function to create a status/info dictionary for a device.

    DEPRECATED: Use UpdateResult.info_record() instead.
    ...
    """
    logger.warning(
        "_create_info_dict() is deprecated. "
        "Use UpdateResult.info_record() instead."
    )
    return UpdateResult.info_record(pf_device, result).to_dict()
